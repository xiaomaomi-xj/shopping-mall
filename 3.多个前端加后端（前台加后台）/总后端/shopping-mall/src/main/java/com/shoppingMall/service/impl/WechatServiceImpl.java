...
public class WechatServiceImpl implements WechatService {
     ...
     */
    @Override
    public List<UserAndWechatBo> getAllWechat() {
        List<UserAndWechatBo> userAndWechatBos = new ArrayList<>();
        List<WechatPo> all = wechatDao.findAll();
        for (WechatPo wechatPo : all) {
            userAndWechatBos.add(new UserAndWechatBo(
                    String.valueOf(wechatPo.getWechatId()),
                    wechatPo.getWechatHeadUrl(),
                    wechatPo.getNickName(),
                    wechatPo.getOpenId(),
                    wechatPo.getSex(),
                    String.valueOf(wechatPo.getUserId()),
                    UserStateEnum.WECHAT.getType()
            ));
        }
     ...
     */
    @Override
    public SingleAllBo<Boolean> delWechat(String id) {
        WechatPo wechatPo = wechatDao.findById(Long.parseLong(id)).orElseThrow(() -> {
            throw new BizException("用户不存在！");
        });
        MyAloneHandlerReadWrite.write(() -> {
            wechatDao.delete(wechatPo);
            return null;
        });
