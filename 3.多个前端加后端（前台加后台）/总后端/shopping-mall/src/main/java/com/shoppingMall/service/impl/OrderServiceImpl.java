...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderServiceImpl(OrderDao orderDao, GoodsDao goodsDao, ImgDao imgDao, MerchantDao merchantDao, ManagerTokenService managerTokenService, MorePayService morePayService) {
    ...
    public List<OrderOnAdminBo> getAllOrderData() {
        ...
        List<OrderOnAdminBo> orderOnAdminBos = new ArrayList<>();
        for (Order order : all) {
            Goods goods = goodsDao.findById(order.getGoodsId()).orElse(null);
            if (goods == null) {
                throw new AppException("数据混乱！");
            }
            String merchantName;
            if (ObjectUtil.isNull(order.getMerchantId())) {
                merchantName = "系统生成订单无商户";
            } else {
                Merchant merchant = merchantDao.findById(order.getMerchantId()).orElse(null);
                if (merchant == null) {
                    throw new AppException("数据混乱！");
                }
                merchantName = merchant.getMerchantName();
            }
            orderOnAdminBos.add(new OrderOnAdminBo(
                    String.valueOf(order.getOrderId()),
     ...
     */
    @Override
    public SingleAllBo<Boolean> modifyOrderShipStatus(Long orderId, int shipStatus) {
        Order order = orderDao.findById(orderId).orElseThrow(() -> {
            throw new BizException("订单不存在！");
        });
        order.setShipStatus(shipStatus);
        MyAloneHandlerReadWrite.write(() -> orderDao.save(order));
